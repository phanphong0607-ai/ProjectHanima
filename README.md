# 🎹 SEEYOUIN4YEARSLATER: Project Hanima

> *"Gratitude is the only thread that connects shattered souls."*

---

## 🌸 Overview
**Project Hanima** is an Indie Pixel Art RPG project inspired by the soul-stirring world of *Undertale*. More than just a game, this project serves as a chronicle of my 4-year academic odyssey at **UTH** (University of Transport and Communications HCMC), charting my path toward becoming a Game Engineer in Japan.

## 🏮 The Lore
The story follows **Hanima**, an ancient Relic crafted in the form of a soulless doll. Following the passing of her guardian—a dedicated Soul Harvester—Hanima experiences a spiritual awakening, manifesting **8 Core Souls** within her vessel. She now embarks on a journey to honor his legacy and fulfill her destiny as **The Soul Liberator**.

## 🛠️ Technical Stack
Developed with a "Low-level" mindset, prioritizing performance optimization and rigorous memory management:
* **Language:** C++20 (The King of Game Development).
* **IDE:** Visual Studio 2026.
* **Objective:** Deepening my understanding of Computer Architecture by building a game system from the ground up.

## 🧭 Progress Log
- [x] **Day 1:** Project initialization & C++20 professional environment setup.
- [x] **Lesson 1.1:** The first lines of code (Statements) - Hanima's Awakening.
- [ ] **Chapter 1.x:** Implementing the Variable system for the 8 Core Qualities.
- [ ] **Chapter 2.x:** Architecting code structure (Functions & Headers).

## ✉️ Contact
* **Developer:** Phong (1st-year IT Student @ UTH)
* **Goal:** Japan Game Studios (Class of 2030)
